...
st.markdown("""
    ...
    /* ランキング順位の強調 */
    .rank-badge {
        background-color: #1a365d; color: white; padding: 2px 8px;
...
school_data = [
    ...
    {"順位": 5, "学校名": "吉祥女子中学校", "偏差値": 63, "カテゴリ": "女子校", "所在地": "武蔵野市", "最寄り": "吉祥寺"},
    {"順位": 6, "学校名": "鴎友学園女子", "偏差値": 62, "カテゴリ": "女子校", "所在地": "世田谷区", "最寄り": "宮の坂"},
    ...
    {"順位": 6, "学校名": "三田国際学園", "偏差値": 61, "カテゴリ": "共学", "所在地": "世田谷区", "最寄り": "用賀"},
]

# メインメニュー
tab_choice = st.radio("メニュー", ["🎓 難関私立中学リスト", "📈 周辺・自治体比較"], horizontal=True, label_visibility="collapsed")
...
if "私立中学" in tab_choice:
    ...
    st.markdown(f"""
        <div style="background-color: {color}; color: white; padding: 10px; border-radius: 10px 10px 0 0; font-weight: bold; text-align: center;">
            {category} 最新ランキング TOP {len(filtered_df)}
        </div>
    """, unsafe_allow_html=True)
    ...
    # 自治体比較タブ
    st.subheader("23区 子育て・教育支援比較")
    st.table(pd.DataFrame({
        "区": ["文京区", "港区", "葛飾区", "世田谷区"],
        "教育特色": ["国立・私立進学率高", "国際教育・ICT", "給食完全無償", "公立小の質が高い"],
        "給食費": ["無償", "無償", "無償", "無償"],
        "高校医療費": ["18歳まで無料", "18歳まで無料", "18歳まで無料", "18歳まで無料"]
    }))
